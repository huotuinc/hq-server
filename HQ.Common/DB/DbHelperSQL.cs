...
namespace HQ.Common.DB
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        public static string connectionString = ConfigHelper.MssqlDBConnectionString_Sync;
        ...
        /// <returns></returns>
        public static DataSet RunProcedureData(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
        ...
        private static DataTable RunDoSplitPageExport(IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                DataSet dataSet = new DataSet();
                ...
                SqlDataAdapter sqlDA = new SqlDataAdapter();
                sqlDA.SelectCommand = BuildQueryCommand(connection, "DoSplitPageExport", parameters);
                sqlDA.Fill(dataSet);
        ...
        public static DataTable GetSplitPageExport(string strSql, int BeginNum, int EndNum, string OrderField, OrderSort Sort)
        {
            SqlParameter[] parameters = {
                                            new SqlParameter("@Sql", SqlDbType.Text),
                                            new SqlParameter("@BeginNum", SqlDbType.Int, 4),
                                            new SqlParameter("@EndNum", SqlDbType.Int, 4),
                                            new SqlParameter("@OrderField",SqlDbType.VarChar,500),
                                            new SqlParameter("@OrderSort",SqlDbType.Int,4)
                                        };
            parameters[0].Value = strSql;
            parameters[1].Value = BeginNum;
            parameters[2].Value = EndNum;
            parameters[3].Value = OrderField;
            parameters[4].Value = (int)Sort;

            DataTable dt = RunDoSplitPageExport(parameters);
            return dt;
        }
        ...
        {
            T model = Activator.CreateInstance<T>();//创建对象实例
            foreach (DataColumn column in row.Table.Columns)
            {
                string columnName = column.ColumnName;
                PropertyInfo prop = model.GetType().GetProperty(columnName);
                if (prop != null)
                {
                    if (row[columnName] != DBNull.Value)
                    {
                        prop.SetValue(model, row[columnName], null);
                    }
                }
            }
            return model;
        ...
        /// <returns></returns>
        public static List<T> GetModelList<T>(DataTable table) where T : new()
        {
            List<T> models = new List<T>();//创建对象集合
