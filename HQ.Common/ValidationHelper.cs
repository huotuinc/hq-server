...
namespace HQ.Common
{
    ...
    public class ValidationHelper
    {
        ...
        #region 验证公交线路是数字还是数字串
        /// <summary>
        /// 验证公交线路是数字还是数字串
        /// </summary>
        /// <param name="srcStr">验证字符串</param>
        /// <param name="IsDigt">验证结果(1:数字,0数字串)</param>
        /// <returns>反回值</returns>
        public static string StringNum(string srcStr, ref int IsDigt)
        {
            //Match m = Regex.Match(srcStr, @"\d＋", RegexOptions.IgnoreCase | RegexOptions.Compiled);
            Match m = Regex.Match(srcStr, @"^[+-]?\d+(\d+)?$", RegexOptions.IgnoreCase | RegexOptions.Compiled);//qkb080125
            if (m.Success)
            {
                IsDigt = 1;
                return m.Value;
