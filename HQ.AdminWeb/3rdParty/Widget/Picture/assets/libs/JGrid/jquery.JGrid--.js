/**
+-------------------------------------------------------------------
* jQuery Grid - 分页列表Jquery插件 - http://www.9126.org/
+-------------------------------------------------------------------
* @version 1.0.0
* @since 2015/08/11
...
(function ($) {
    ...
    $.fn.Grid = function (options) {
            ...
            if ($this.rows != null && $this.rows.length > 0) {
                ...
                for (var i = 0; i < $this.rows.length; i++) {
                    if ($this.showOrder) {//表头点击排序功能
                        if ($this.rows[i].search != undefined) {
                            var _id = "jackson-rows-title-" + $this.rows[i].field;
                            var _searchHtml = '<div class="jackson-input"><input class="jackson-btn jackson-btn-input" id=' + ((_id) + '_' + $this.rows[i].search) + ' type="text"><a href="javascript:void(0);" class="jackson-btn jackson-btn-search">搜</a></div>';
                            shtml += "<th style='cursor:pointer;' jacksonrowsqx='0' id='" + _id + "' onclick=\"$.__orderGrid('" + $this.rows[i].order + "','" + _id + "','" + $this.rows[i].title + "','" + div + "')\" align=\"left\" width=\"" + $this.rows[i].width + "\">" + _searchHtml + "</th>";
                        }
                        else {
                            var _id = "jackson-rows-title-" + $this.rows[i].field;
                        ...
                        if ($this.rows[i].search != undefined) {
                            var _id = "jackson-rows-title-" + $this.rows[i].field;
                            var _searchHtml = '<div class="jackson-input"><input  class="jackson-btn jackson-btn-input" id=' + ((_id) + '_' + $this.rows[i].search) + ' type="text"><a href="javascript:void(0);" class="jackson-btn jackson-btn-search">搜</a></div>';
                            shtml += "<th align=\"left\" width=\"" + $this.rows[i].width + "\">" + _searchHtml + "</th>";
                        }
                        else {
        ...
        self.resolvePager = function (option) {
            var $this = option;
        ...
        self.HttpGet = function (element, option) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var pagerHtml = element.resolvePager(obj);//获取table pager部分
                    shtml += tableHead;
                    shtml += bodyhtml;
                    shtml += "</tbody>";
                    shtml += "</table>";
