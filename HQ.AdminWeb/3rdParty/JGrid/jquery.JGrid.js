...
(function ($) {
        ...
        __checkAll: function (ck, div) {
            ...
            $("#jackson-data-datatable-box-" + $this.name + " td input[type=checkbox]").each(function (idx, item) {
                if (ck.checked) {
                    ...
                    item.checked = ck.checked;
                    _calCheckValue += $(item).val() + ',';
                    _checkvalue = "";
                }
            });
            $this.checkValue = _checkvalue.toString().substr(0, _checkvalue.length - 1);
            _calCheckValue = _calCheckValue.toString().substr(0, _calCheckValue.length - 1);
            ...
            if (typeof $this.selectAll === 'function') {
                if ($this.checkCount > 0) {
                    $this.selectAll($this.checkValue, $this.checkCount);
                } else {
                    $this.selectAll(_calCheckValue, $this.checkCount);
                }
            }
            $("#" + div).data('Grid', $this);
        },
    ...
    $.fn.Grid = function (options, callback) {
        ...
        /***************************************共用方法开始******************************************************/
        self.resolveHeader = function (option) {
                        ...
                        if (obj.doubleLine && (i + 1) % 2 == 0) {
                            if (bgc != '') {
                                if (obj.showCheck) {
                                    ...
                                    _body += "<tr onclick=\"$.__OnGridDelegate('" + div + "','" + i + "')\" bgcolor=\"" + bgc + "\" class=\"jackson-data-rows-datarow\">";
                                ...
                                if (obj.showCheck) {
                                    ...
                                    _body += "<tr onclick=\"$.__OnGridDelegate('" + div + "','" + i + "')\" bgcolor=\"#f2f2f2\" class=\"jackson-data-rows-datarow\">";
                                }
                                else if (obj.showRadio) {
        ...
        */
        ...
        $.fn.Jump = function (current, pageSize, recordCount) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (obj.isTemplate) {//模版展示方式
                        ...
                        var tableHead = element.resolveHeader(option);//获取解析table头
                        var pagerHtml = element.resolvePager(obj, data.Total);//获取table pager部分
                        shtml += tableHead;
                        shtml += bodyhtml;
                        shtml += "</tbody>";
        ...
        */
        ...
        self.pagerGridHtml = function (option) {
            ...
            if (totalPages >= 1) {
                ...
                _htmlPager += "<li><input type=\"text\" class=\"jackson-input\" id=\"jackson-data-rows-page-current-" + obj.label + "\" value=\"" + currentPage + "\"/></li>";
                _htmlPager += "<li><a href=\"javascript:$('#" + obj.label + "').Jump(\'jackson-data-rows-page-current-" + obj.label + "\',\'" + pageSize + "\',\'" + totalPages + "\')\" title='跳转'>跳转</a></li>";
